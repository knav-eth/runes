...
export function useAvailableWalletNs(cacheKey?: any): UseWalletNsValue {
  ...
  const checkAvailability = useCallback(async (tokenId: number): Promise<boolean> => {
  ...
  const fetchWalletNs = useCallback(async (address: string): Promise<void> => {
    ...
    const nsWithAvailability = await Promise.all(walletNs.map(async (n): Promise<NWithAvailability> => {
